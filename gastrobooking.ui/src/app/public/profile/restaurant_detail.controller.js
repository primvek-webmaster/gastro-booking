...
(function () {
    ...
    /*@ngNoInject*/
    function RestaurantEditController($state, $scope, Cropper, $timeout, appConstant, $location, $anchorScroll, $stateParams, $rootScope, $translate, ProfileService) {
        var vm = this;
        ...
        function initialTimeSetup(){
            var keys = Object.keys(vm.time);
            for (var i = 0; i < keys.length; i++){
                vm.time[keys[i]].m_start = vm.time[keys[i]].a_start = $translate.instant('RESTAURANT_DETAIL.FROM');
                    vm.time[keys[i]].m_end = vm.time[keys[i]].a_end = $translate.instant('RESTAURANT_DETAIL.UNTIL');
