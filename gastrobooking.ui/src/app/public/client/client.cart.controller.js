...
(function () {
    ...
    function ClientCartController($state, $rootScope,$scope,$timeout,ClientCartService,ClientService, uiDatetimePickerConfig, $translate, $stateParams, moment, appConstant, RestaurantDetailService, $geolocation, AuthService) {
        ...
        vm.getQuizPrize = getQuizPrize;
        vm.getQuizClient = getQuizClient;

        vm.changedDateTime = false;
        vm.firstDatePicker = -1;
        vm.onChangeServeTime = onChangeServeTime;
        vm.changeAllServeTime = changeAllServeTime;
        ...
        $scope.calculateDiscount = function(){
            ...
            if(vm.discount_codes != null && vm.discount_codes.length > 0){
                ...
                for (var i = 0; i < vm.discount_codes.length; i++) {
                    var value = vm.discount_codes[i];
                    if(value.code === vm.entered_discount_code && value.used === null && vm.orderDetail[0] && value.currency == vm.orderDetail[0].menu_list.data.currency){
            ...
            if(discount_per && !discount_amount){
                discount_amount = parseInt(vm.totalPrice) * discount_per / 100;
            }
            else if(discount_per && discount_amount){
                discount_amount = (parseInt(vm.totalPrice) * discount_per / 100) + parseInt(discount_amount);
            }

            if (parseInt(discount_amount) > parseInt(vm.quizSetting.quiz_prize)) {
                vm.order.gb_discount = Math.floor(parseInt(vm.quizSetting.quiz_prize) / parseInt(vm.totalPrice) * 100);

                discount_amount = parseInt(vm.totalPrice) * parseInt(vm.order.gb_discount) / 100;
            }
            ...
            if ( parseInt(discount_amount ) > parseInt(vm.totalPrice) ){
                ...
                if(vm.orderDetail.length){
                    // vm.totalPrice = vm.order.total_price;
                    calculate = parseInt( vm.totalPrice ) - parseInt( discount_amount == null ?  0 : discount_amount) + ' ' + vm.orderDetail[0].menu_list.data.currency;
                }
            }

            vm.discount_amount = discount_amount ? discount_amount : 0;
        ...
        $scope.$watch('vm.order.gb_discount', function(nV){
            ...
            if(!vm.quizDiscount.percentage_discount){
                vm.order.gb_discount = 0;
            }
            else if(newValue > vm.quizDiscount.percentage_discount){
                vm.order.gb_discount = vm.quizDiscount.percentage_discount;
            }
            else if(newValue < 0){
                vm.order.gb_discount = 0;
            }
        ...
        function getOrder(orderId) {
            ...
            ClientService.getOrder(orderId).then(function (response) {
                ...
                var currentLanguage = vm.order.restaurant_lang == 'ENG' ? 'en' : 'cs';
        ...
        function placeOrder(isValid) {
            ...
            if (isValid)
            {
                ...
                if (vm.apply_code || vm.discountId > 0) {
                    vm.order.code = vm.entered_discount_code;
                } else {
                    vm.order.code = null;
                }
                vm.order.persons = vm.persons ? vm.persons : 0;
                vm.order.ID_tables = vm.ID_tables && vm.ID_tables > 0 ? vm.ID_tables : null;

                changeOrdersServeAt();
                calculateCommission();

                vm.order.total_price = vm.totalPrice;
                vm.order.discount_amount = vm.discount_amount;
        ...
        function deleteOrderFromCart(order) {
            ...
            var msg = $translate.instant('CART.CONFIRM_CANCEL_MESSAGE');
        ...
        function saveChanges(isValid, changeState) {
            ...
            if (vm.orderDetail.length > 0 && changeState){
                if (!checkTableInterval()) {
                    return;
                }
                vm.loading = true;
                ...
                var order = {
                    ...
                    "lang": lang,
                    "discountId": vm.discountId,
                    "gb_discount": vm.order.gb_discount
                };
                ...
                var payment = {
                    "ID_orders":vm.orderDetail[0].ID_orders,
                    "datetime":vm.orderDetail[0].serve_at,
                    "type":2,
                    "amount":vm.order.gb_discount,
                    "last_update":vm.orderDetail[0].serve_at
                };
                ...
                ClientService.updateOrder(order).then(function (response) {
                    ...
                    if (changeState){
                        ...
                        var length = vm.orderDetail.length;
                        var serve_at = vm.orderDetail[length - 1].serve_at;
                        localStorage.setItem('high_serve_at', serve_at);
        ...
        function changeOrdersServeAt() {
            for (var i = 0; i < vm.orderDetail.length; i++){
                if (vm.orderDetail[i].side_dish_bool) {
                    for (var j = 0; j < vm.orderDetail.length; j++){
                        if (vm.orderDetail[j].ID_orders_detail == vm.orderDetail[i].side_dish) {
                            vm.orderDetail[i].serve_at = vm.orderDetail[j].serve_at;
                            break;
                        }
                    }
                }
        ...
        function getMenuList() {
            angular.forEach(vm.orderDetail, function(detail) {
                var currency = detail.currency;
                var id_menu_list = detail.ID_menu_list;
                var req = {
                    "id": id_menu_list,
                    "currency": currency
                };
                RestaurantDetailService.getMenuList(req).then(function (response) {
                    ...
                    detail.commission_split = parseFloat(data.setting[0].commission_split ? data.setting[0].commission_split : 0);
                });
            });
        ...
        function calculateCommission() {
            ...
            var gb_discount = parseInt(vm.discount_amount) / parseInt(vm.totalPrice) * 100;
            for (i = 0; i < vm.orderDetail.length; i++) {
                var discount_amount = parseInt(vm.orderDetail[i].t_price) * gb_discount / 100;

                vm.orderDetail[i].client_commission = (vm.orderDetail[i].t_price - (discount_amount > 0 ? discount_amount : 0)) * vm.orderDetail[i].menu_list.data.client_percentage * vm.orderDetail[i].commission_split / 100 / 100;
                vm.orderDetail[i].member_commission = (vm.orderDetail[i].t_price - (discount_amount > 0 ? discount_amount : 0)) * vm.orderDetail[i].menu_list.data.client_percentage * (1 - vm.orderDetail[i].commission_split / 100) / 100;
            }
        }
        ...
        function getSumPrize() {
            ...
            if (vm.order) {
                // vm.order.gb_discount = percentage_discount;
            }

            console.log(vm.quizDiscount);
        }
        ...
        var saved = false;
        function onChangeServeTime(index) {
            // if (!saved) {
                vm.firstDatePicker = index;
            // }
            vm.changedDateTime = true;
        }

        function changeAllServeTime() {
            for (var i = 0; i < vm.orderDetail.length; i++) {
                if (i != vm.firstDatePicker) {
                    vm.orderDetail[i].serve_at = vm.orderDetail[vm.firstDatePicker].serve_at;
                }
            }
            vm.changedDateTime = false;
            saved = true;
