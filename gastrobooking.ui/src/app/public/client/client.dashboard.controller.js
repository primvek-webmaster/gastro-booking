...
(function () {
    ...
    /*@ngNoInject*/
    function ClientDashboardController($rootScope, $scope, $timeout, ClientService, CoreService, $state, $stateParams, $translate, $interval, $filter, $geolocation, $sce, $location, appConstant) {
        ...
        if (localStorage.getItem('requestData')) {
            ...
            if (requestData.restaurantData) {
                if (item.street && item.city) {
                    var restaurantAddress = item.street + ', ' + item.city;
                }
                else if (item.street) {
                    var restaurantAddress = item.street;
                }
                else if (item.city) {
                    var restaurantAddress = item.city;
                }
                else {
        ...
        /*...Rating.......*/
        $scope.init_rating = function (order_index) {
            if (!vm.orders[order_index].rated) {
                /* data structure */
                var RateDetailClass = {
                    collapsed: true,
                    ID_orders_detail: 0,
                ...
                var RateClientClass = {
                    ...
                    rate_order: {
                        ...
                        surround_rating: 5,
                        menu_rating: 5,
                        service_collapsed: true,
                        surround_collapsed: true,
                        menu_collapsed: true,
                        rate_surround: {
                            pleasant_rating: 5,
                            toilet_rating: 5,
                ...
                var order = vm.orders[order_index];
                var rate_client = angular.copy(RateClientClass);

                rate_client.ID_orders = order.ID_orders;

                angular.forEach(order.orders_detail[Object.keys(order.orders_detail)[0]], function (order_detail, index) {
                    var rate_detail = angular.copy(RateDetailClass);
                    rate_detail.ID_orders_detail = order_detail.ID_orders_detail;
                    rate_detail.name = order_detail.menu_list[Object.keys(order_detail.menu_list)[0]].name;
                    rate_client.rate_order.rate_detail.push(angular.copy(rate_detail));
                });
                vm.orders[order_index].rate_client = angular.copy(rate_client);
            }
        };

        $scope.cancel_rating = function (order_index) {
            if (vm.orders[order_index].rated) {
                vm.orders[order_index].rate_client = null;
            }
        };

        $scope.save_rating = function (order_index) {
            var order = vm.orders[order_index];
            var data = order.rate_client;
            vm.loadingSaveChanges = true;
            ClientService.orderRate(data).then(function (response) {
                angular.element('#orderRate' + order_index).modal('hide')
                order.rated = true;
                vm.loadingSaveChanges = false;
            });
        };
        ...
        $scope.addMeal = function () {
            if ($scope.request.request_side_dish == true) {
                var side_dish = 'Y';
            }
            else {
                var side_dish = 'N';
            ...
            if ($scope.request.request_friend_name) {
                var friend_id = $scope.request.request_friend_name;
            }
            else {
                var friend_id = null;
            }
            ...
            if (meal) {
                ...
                if (!$scope.request_ingredient) {
                    $scope.request_ingredient = {};
                }
        ...
        $scope.sendRequest = function () {
            ...
            if (sendRequestDetails) {
                var sendRequestDetails = angular.toJson(sendRequestDetails)
            } else {
                var sendRequestDetails = []
            }
        ...
        $scope.requestSaveChanges = function (index, requestId, orderId) {
            ...
            orderDetailObj = [];
            if (requestConfirm) {
                var requestConfirm = angular.toJson(requestConfirm)
            } else {
                var requestConfirm = []
            }
                                            ...
                                            if (date1 < currentTime) {
                                                if (date1 > vm.last_pay_date) {
                                                    if (order[j].currency == client_currency) {
                                                        ...
                                                        member_commission = member_commission + (order[j].member_commission ? Number(order[j].member_commission) : 0);
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    if (date1 < currentTime) {
                                        if (date1 > vm.last_pay_date) {
                                            if (order[j].currency == client_currency) {
                                                ...
                                                client_commission = client_commission + (order[j].client_commission ? Number(order[j].client_commission) : 0);
                                            }
                                        }
                                    }
                                }
        ...
        function orders_sum_price_between_dates() {
            ...
            var currentLanguage = localStorage.getItem('current_language_code');
            ClientService.orders_sum_price_between_dates({
                'step': vm.percentage,
                'lang': currentLanguage,
                'now': $filter('date')(date2, 'yyyy-MM-dd HH:mm:ss')
                ...
                if (vm.isBonusQuiz && vm.sum_price.bonus_quiz_count == 0) {
                    vm.isNextQuiz = false;
                } else if (!vm.isBonusQuiz && vm.sum_price.bonus_quiz_count == 0) {
                    vm.isNextQuiz = false;
                } else if (!vm.isBonusQuiz && vm.sum_price.bonus_quiz_count > 0) {
                    vm.isNextQuiz = true;
                    vm.isBonusQuiz = true;
        ...
        function nextQuestion() {
            if (vm.isNextQuiz) {
                ...
                var msg = $translate.instant('CLIENT.CANNOT QUIZ');
                alert(msg);
            }
        }
        ...
        function getbBookings() {
            ...
            ClientService.getOrders(vm.currentPage).then(function (response) {
                ...
                for (var i = 0; i < vm.orders.length; i++) {
                    vm.orders[i].gb_discount = parseFloat(vm.orders[i].gb_discount);
                    vm.orders[i].real_price = parseFloat(vm.orders[i].total_price) - parseFloat(vm.orders[i].gb_discount);
        ...
        function getDiets() {
            ClientService.getDiets().then(function (response) {
                ...
                angular.forEach($rootScope.availableLanguage, function (value, key) {
                    vm.diets[value.code] = {'null': {'id': '-1', 'name': "No diet", 'cust_order': 0, 'lang': "none"}};
                });
        ...
        function saveClient(isValid) {
            if (isValid) {
                ...
                ClientService.updateClient(vm.myDetails).then(function (response) {
                    ...
                    vm.myDetailsSuccess = $translate.instant("CLIENT.CHANGE SUCCESS");
        ...
        function getRestaurantType() {
            ...
            CoreService.getRestaurantTypes().then(function (response) {
                vm.business_type = response.data;

                angular.forEach(vm.business_type, function (value, key) {
        ...
        function getBillingPatronage() {
            ...
            ClientService.getBillingPatronage(vm.currentPage, now).then(function (response) {
                ...
                var accountNumber = vm.myDetails.account_number + vm.myDetails.bank_code;
