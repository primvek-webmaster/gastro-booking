    ...
    function RestaurantSearchController($scope, $state, $stateParams, $rootScope, SearchService, $translate, appConstant, CoreService, paginationService) {
        ...
        function search(isValid) {
            ...
            if (isValid){
                ...
                SearchService.searchRecommended(searchParam, currentPage).then(function(response){
                    if(response.data.length!=0){
                        if (searchParam.searchToggle){
                            ...
                            vm.restaurantRecommended = response.data[0];
                            if (vm.restaurantRecommended.logo){
                                vm.restaurantRecommended.logo = appConstant.imagePath + vm.restaurantRecommended.logo;
                            } else {
                                vm.restaurantRecommended.logo = "assets/images/placeholder.jpg";
                            }
                            vm.loading = false;
                        }
                    }
