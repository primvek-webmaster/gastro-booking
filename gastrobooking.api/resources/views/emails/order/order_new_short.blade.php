...
<body bgcolor="#FFFFFF" lang=CSlink=#000080 vlink=#800080 text="#000000">
	...
	<p>
		<?php echo ($order->pick_up === 'Y') ? Lang::get("main.MAIL.PICK UP") : (($order->delivery_address && $order->delivery_phone) ? Lang::get("main.MAIL.DELIVERY") : Lang::get("main.MAIL.BOOKING FOR")); ?> <?= $order->cancellation  ?>  <?= $user->name ?> <?= $client_number ?> <?= $order->delivery_address; ?><br/>
		...
		<?=  $orders_detail->x_number ?>@lang('main.MAIL.AVAILABLE SYMBOL') <?= $orders_detail->menu_list->prefix ?> <?= $orders_detail->menu_list->name ?>  <?= $orders_detail->price ?> <?= $orders_detail->menu_list->currency ?> @lang('main.MAIL.AVAILABLE SYMBOL') <?= $orders_detail->x_number ?>,<br/>
