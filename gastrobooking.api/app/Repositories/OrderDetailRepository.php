...
//-------------------------------------------------------------------------------
    ...
    public function all($restaurantId){
        ...
        if ($order) {
            ...
            $orders_detail = $orders_detail->sortBy(function($order_detail) {
                return sprintf('%s', $order_detail->serve_at);
    ...
    public function sendEmailReminder($type, User $user, Order $order, $restaurant, $lang, $orders_detail_filtered, $to)
    {
        ...
        if ($to == 'rest') {
            $path = 'emails.order.restaurant_order_' . $type;
        } else {
            $path = 'emails.order.order_' . $type;
        }

        $orders_detail_count = 0;
        $orders_detail_total_price = 0;
        foreach ($order->orders_detail as $order_detail) {
            if ($order_detail->status != 3){
                if ($order_detail->is_child){
                    $orders_detail_total_price += ($order_detail->menu_list->price_child && $order_detail->menu_list->price_child > 0 ? $order_detail->menu_list->price_child : $order_detail->menu_list->price) * $order_detail->x_number;
                } else {
                    $orders_detail_total_price += $order_detail->menu_list->price * $order_detail->x_number;
                }
                $orders_detail_count ++;
            }
        ...
        $confirm_url = App::make('url')->to('/api/confirm_order?order_id='.$order->ID);

        try {
            Mail::send($path,
                ['user' => $user, 'order' => $order, 'restaurant'=> $restaurant,
                    'orders_detail_count'=> $orders_detail_count, 'orders_detail_filtered' => $orders_detail_filtered, 'confirm_url' => $confirm_url,
                    'orders_detail_total_price' => $orders_detail_total_price],
